<template>
    <form class="xl:w-1/3 mx-auto" @submit.prevent="getShortLink">
        ...
        <div class="inline-flex border-solid border-2 rounded-md overflow-hidden">
            <span class="py-4 px-1 bg-blue-500 text-white"> {{ `${navigation?.location?.protocol}//${navigation?.location?.host}` }}/ </span>
            <input v-model="shortCode" type="text" class="px-2 inline-block w-40" placeholder="short-code" aria-label="url field"
                aria-describedby="paste your url here" />
        </div>
...
const getShortLink = async ():Promise<void> => {    
    try {
        ...
        let res = await fetch(`${apiBase}/url-shortener/alias`, {
            method: "POST",
